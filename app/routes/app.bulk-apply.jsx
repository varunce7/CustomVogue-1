    ...
    <div style={s.page}>
      ...
      {result?.success && (
        <div style={s.successBanner}>
          ...
          Fields copied to<strong style={{ margin: "0 -4px" }}>{result.copied}</strong> product{result.copied !== 1 ? "s" : ""} successfully!
        </div>
      )}
      {result?.warning && (
      ...
      <form onSubmit={handleSubmit}>
        ...
        {sourceProduct && targetProducts.length > 0 && (
          <div style={s.summaryBox}>
            ...
            Will copy fields from <strong>{sourceProduct.title}</strong> to{" "}
            <strong>{targetProducts.length} product{targetProducts.length !== 1 ? "s" : ""}</strong>.
          </div>
        )}
